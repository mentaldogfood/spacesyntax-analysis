...
from modules.extract_bia_boundaries import (
    OUTPUT_ROOT,
    TORONTO_CRS,
    BIA_BUFFER_M,
    extract_bia,
...
from modules.depthmapx_segment import (
    DEFAULT_EXE,
    add_derived_metrics,
    build_geodataframe,
    export_results,
...
from modules.citywide_match import (
    CITYWIDE_CSV,
...
def parse_args() -> argparse.Namespace:
    ...
    g.add_argument("--citywide",  action="store_true",
                   help="Run the full pipeline on the entire road network, no BIA clipping")
    ...
    p.add_argument("--citywide-timeout", type=float, default=7200.0,
                   help="depthmapXcli per-step timeout in seconds for --citywide mode "
                        "(the full network is much larger than a single BIA)")
    return p.parse_args()
...
def _run_citywide(
    ...
    """
    Run the full pipeline on the entire, unclipped road network.
    Same export format as a single-BIA run (_run_one), just without the
    BIA-boundary clip step.
    """
    slug       = "Citywide"
    output_dir = args.output_dir or (OUTPUT_ROOT / slug)
    exe        = args.depthmapxcli
    ...
    try:
        with contextlib.redirect_stdout(_sink):
            dxf, _ = extract_citywide(output_dir)

            out_graph = output_dir / f"{slug}_segment_map.graph"
            result_df = run_depthmapxcli(dxf, exe, label=slug, out_graph=out_graph,
                                          timeout=args.citywide_timeout)
            print(f"  {len(result_df):,} segments\n")
            ...
            gdf = gdf.copy()
            gdf.insert(0, "bia", slug)

        return summary, gdf

    except Exception as e:
        print(f"  FAILED [citywide]: {e}", file=sys.stderr)
        return None, None
...
def main() -> None:
    ...
    if args.citywide:
        print("Running pipeline for the entire road network (no clipping)...\n")
        summary, _ = _run_citywide(args, citywide_gdf, quiet=False)
        if summary is None:
            sys.exit(1)
        print("\nAll done.")
        sys.exit(0)
    ...
    if not args.bia:
        sys.exit("ERROR: --bia, --all-bias, or --citywide is required.  Use --list-bias to see options.")
